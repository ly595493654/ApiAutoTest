...
def test_003(mock_pay):
    pay = Pay()
    canshu = {"订单号": "12345", "支付金额": 20000.56, "支付方式": "微信"}
    r = pay.zhifu(canshu)
    print(r)
    assert r['msg'] == "支付成功"
...
# 取现接口未实现,写一个取现成功的用例.
def withraw(data):
    r = requests.post("http://jy001:8081/futureloan/mac/api/member/withdraw", data=data)
    print("实际返回值",r.text)
    return r.json()
...
def test_withraw():
    ...
    withraw = mock.Mock(return_value = {"status": 1, "code": "20001", "data": None, "msg": "取现成功"})
    r = withraw(canshu)
    print("mock后的返回值",r)
    print(r['msg'])
